...
- Commands: `src/commands/{run,list,which,doctor,init,create,select}.ts`
...
- Tests must set `AIUSE_CONFIG` env var — never touch `~/.aiswitch` directly
- Tests must set `AIUSE_LAST_USED` env var — never touch `~/.aiswitch/last-used.json` directly
